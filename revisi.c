...
static const char *dirpath = "/home/safhiram/Shift4/test_shift4";
		       ...
		       off_t offset, struct fuse_file_info *fi)
{
	...
	while ((de = readdir(dp)) != NULL) {
			...
			&& ((tmp.st_mode & S_IRUSR) == 0 || (tmp.st_mode & S_IRGRP) == 0 || (tmp.st_mode & S_IROTH) == 0) )
		{
			...
			char buffer[1000];
			sprintf(buffer,"%s-%d-%d-%s",katak,name->pw_uid,grup->gr_gid,t);
			fprintf(fp,"%s\n",buffer);
			remove(file);
			fclose(fp);
